...
fun excluirLivro(repositorio: MutableList<Livro>) {
    ...
    val livroRemovido = repositorio.remove(livro)
    if (livroRemovido) {
...
fun dadosEdicao(atributo: String, repositorio: MutableList<Livro>) {
...
fun editarLivro(repositorio: MutableList<Livro>) {
    ...
    while (opcao != 3) {
        ...
        when (opcao) {
            1 -> dadosEdicao("nome", repositorio)
            2 -> dadosEdicao("preco", repositorio)
        }
    }
}

fun listar(repositorio: MutableList<Livro>) {
    for (livro: Livro in repositorio) {
        println(livro)
    }
